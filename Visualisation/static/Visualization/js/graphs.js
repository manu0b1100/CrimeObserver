...
function makeGraphs(error, recordsJson) {
    ...
    var psDim = data.dimension(function (d) {
        return d["police_station"]
    });
    var disDim = data.dimension(function (d) {
        return d["district"]
    });

    var offenceDim = data.dimension(function (d) {
        return d["final_charge"]
    });
    var timeDim = data.dimension(function (d) {
        return d["hour"]
    });
    ...
    var drawMap = function () {
        ...
        _.each(allDim.top(Infinity), function (d) {
            geoData.push([d["latitude"], d["longitude"], "Offence:" + d["final_charge"] + "<br>Date:" + d["date"] + "<br>Time:" + d["time"] + "<br>Police Station:" + d["police_station"] + "<br><a href=" + d["link"] + ">Fir Link</a>"]);
        });
        var geoData2 = [];
        _.each(allDim.top(Infinity), function (d) {
    ...
    dcCharts = [disChart, pstationChart, offenceChart, timeChart, dateChart];
